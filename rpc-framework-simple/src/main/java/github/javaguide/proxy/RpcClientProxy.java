...
/**
 * Dynamic proxy class. When a dynamic proxy object calls a method, it actually calls the following
 * invoke method. It is precisely because of the dynamic proxy that the remote method called by the
 * client is like calling the local method (the intermediate process is shielded)
 *
 * @author shuang.kou
...
public class RpcClientProxy implements InvocationHandler {
  ...
  public RpcClientProxy(
      RpcRequestTransport rpcRequestTransport, RpcServiceConfig rpcServiceConfig) {
  ...
  /**
   * This method is actually called when you use a proxy object to call a method. The proxy object
   * is the object you get through the getProxy method.
   */
  @SneakyThrows
  @SuppressWarnings("unchecked")
  @Override
  ...
  private void check(RpcResponse<Object> rpcResponse, RpcRequest rpcRequest) {
    ...
    if (rpcResponse.getCode() == null
        || !rpcResponse.getCode().equals(RpcResponseCodeEnum.SUCCESS.getCode())) {
      throw new RpcException(
          RpcErrorMessageEnum.SERVICE_INVOCATION_FAILURE,
          INTERFACE_NAME + ":" + rpcRequest.getInterfaceName());
    }
